...
The convention is to write \( m \) for the \( \hatL_z \) eigenvalue (the magnetic quantum number), and \( j \), the azimuthal quantum number, to describe the \( j(j+1) \) eigenvalue of the \( \Lcap^2 \) operator.
%
...
\index{angular momentum!azimuthal quantum number}
